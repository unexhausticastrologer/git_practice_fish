...
[My GitHub Repository](https://github.com/unexhausticastrologer/git_practice_fish)
...
Github is designed to monitor the change of the edition. It is not convenient to backup single specific document. it just not designed for the backup.
```
...
# Write your answer here
That chunk will be not shown in the output html document.
```
...
path <- "./output/data/processed_data.csv"
...
Cdata <- data.frame(NN_data[,c(3:8)], stringsAsFactors = TRUE)
# Check spelling in species and treatment but also generate a summary table
levels(factor(Cdata$species))
levels(factor(Cdata$treatment))
...
```{r}
st.sum <- ddply(Cdata, c('treatment', 'species'), summarize, n = length(activity), mean = mean(activity), sd = sd(activity))
st.sum
```
...
#The green highlighted line means the including which is the same. The red highlighted line means the including which is the different.
```
...
ggplot(data = Cdata, mapping = aes(x = treatment, y = activity, colour=treatment)) +
  geom_point() + geom_boxplot() + facet_wrap(.~species)
```
...
ggsave("general.png", plot = last_plot(), path = "./output/figures/")
```
...
## Write your answer here
# just write the "output/figures/" to the document directly.
```
...
I want to track the change because if these picture changed, the plot will be also changed.
```
...
Cdata %>% filter(species == c('chromis', 'lemon', 'acantho')) %>% ggplot(mapping = aes(x = treatment, y = activity, colour=treatment)) +
  geom_point() + geom_boxplot() + facet_wrap(.~species)
...
```{r}
fish.aov <- aov(activity~species*treatment, data=Cdata)
fish.aov
```
...
```
```{r}
fish.hsd <- TukeyHSD(fish.aov)
C <- par(mfrow=c(1,3))
plot(fish.hsd)
```
```{r,eval=FALSE}
# I am sorry that I just do not want to describe them independently. I think the plots are clearly enough:)
```
